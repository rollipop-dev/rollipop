---
title: Reporter
---

Reporters allow you to receive and handle build events, HMR failures, progress updates, and client logs from your Rollipop application. This is useful for custom logging, monitoring, or integrating with external tools.

## Reporter Interface

To create a custom reporter, implement the `Reporter` interface:

```ts
interface Reporter {
  update(event: ReportableEvent): void;
}
```

`ReportableEvent` includes every event published in the current Rollipop process. Standalone builds publish build events, while a dev server also publishes HMR, client, and server events.

When an event comes from a dev-server bundler instance, it includes `bundlerId`, which identifies the bundler that produced the event.

## Event Types

### Build Events

#### `bundle_build_started`

Emitted when a bundle build starts.

```ts
{
  type: 'bundle_build_started';
  bundlerId?: string;
}
```

#### `bundle_build_done`

Emitted when a bundle build completes successfully.

```ts
{
  type: 'bundle_build_done';
  bundlerId?: string;
  totalModules: number;
  transformedModules: number;
  cacheHitModules: number;
  duration: number;
  bundleFilePath?: string;
}
```

#### `bundle_build_failed`

Emitted when a bundle build fails with an error. HMR update failures are reported as `hmr_failed` instead.

```ts
{
  type: 'bundle_build_failed';
  bundlerId?: string;
  error: Error;
}
```

### HMR Events

#### `hmr_updates`

Emitted when HMR updates are ready to send to connected clients.

```ts
{
  type: 'hmr_updates';
  bundlerId: string;
  updates: BindingClientHmrUpdate[];
  changedFiles: string[];
}
```

#### `hmr_failed`

Emitted when an HMR update fails with an error.

```ts
{
  type: 'hmr_failed';
  bundlerId?: string;
  error: Error;
}
```

### Transform Events

#### `transform`

Emitted during module transformation, providing progress information.

```ts
{
  type: 'transform';
  bundlerId?: string;
  id: string; // Module ID being transformed
  totalModules: number | undefined; // Total number of modules
  transformedModules: number; // Number of modules processed for progress
}
```

### Watch Events

#### `watch_change`

Emitted when a file change is detected in watch mode.

```ts
{
  type: 'watch_change';
  bundlerId?: string;
  id: string; // Path of the changed file
}
```

### Client Log Events

#### `client_log`

Emitted when the client application logs to the console. This is compatible with Metro's client log format.

```ts
{
  type: 'client_log';
  bundlerId?: string;
  level: 'trace' | 'info' | 'warn' | 'log' | 'group' | 'groupCollapsed' | 'groupEnd' | 'debug' | 'error';
  data: any[];  // Arguments passed to console method
}
```

### Dev Server Events

When running a dev server, reporters also receive client lifecycle events (`client_connected`, `client_message`, `client_error`, and `client_disconnected`) and server events (`server_ready` and `cache_reset`).

## Custom Reporter Example

Here's an example of a custom reporter that sends build events to a monitoring service:

```ts title="my-reporter.ts"
import type { Reporter, ReportableEvent } from 'rollipop';

export class MonitoringReporter implements Reporter {
  update(event: ReportableEvent): void {
    switch (event.type) {
      case 'bundle_build_started':
        this.sendMetric('build.started');
        break;

      case 'bundle_build_done':
        this.sendMetric('build.completed', {
          transformedModules: event.transformedModules,
          cacheHitModules: event.cacheHitModules,
        });
        break;

      case 'bundle_build_failed':
        this.sendMetric('build.failed', {
          error: event.error.message,
        });
        break;

      case 'hmr_failed':
        this.sendMetric('hmr.failed', {
          error: event.error.message,
        });
        break;

      case 'hmr_updates':
        this.sendMetric('hmr.updates', {
          updates: event.updates.length,
          changedFiles: event.changedFiles,
        });
        break;

      case 'transform':
        if (event.totalModules) {
          const progress = (event.transformedModules / event.totalModules) * 100;
          this.sendMetric('build.progress', { progress });
        }
        break;

      case 'watch_change':
        console.log(`File changed: ${event.id}`);
        break;

      case 'client_log':
        this.sendLog(event.level, event.data);
        break;
    }
  }

  private sendMetric(name: string, data?: any) {
    // Send to your monitoring service
    console.log(`[Metric] ${name}`, data);
  }

  private sendLog(level: string, data: any[]) {
    // Send client logs to your logging service
    console.log(`[Client ${level}]`, ...data);
  }
}
```

Use your custom reporter in the config:

```ts title="rollipop.config.ts"
import { defineConfig } from 'rollipop';
import { MonitoringReporter } from './my-reporter';

export default defineConfig({
  reporter: new MonitoringReporter(),
});
```
